<?php
  ...
  $bdd=base_donnees();
   ...
   $dateAujourdhui=new DateTime("now");
    ...
    if(isset($_GET['idRDV'])){
        $idRDV=$_GET['idRDV'];
        $req2=$bdd->prepare('SELECT idRdv, rdvConfirme FROM rdv WHERE idRdv=:idRDV');
         $req2->bindValue('idRDV',$idRDV);
         $req2->execute();
         $infos=$req2->fetchALL(PDO::FETCH_ASSOC);
         $confirme=$infos[0]['rdvConfirme'];
     }
              ...
              <?php if($type=='reserve'):?>
                ...
                <i class="far fa-window-close position-absolute" style="right: 12px;top:437px;"></i>
                    <?php if($confirme =='true'): ?>
                        <input type="submit" name="acceder" value="accéder au dossier médical" class="btn btn-success position-absolute" style="left: 1px;width:220px">
