<?php
    ...
    if(isset($_GET["etat"]))
    {
        $idSuivi=$_SESSION["idSuivi"];
        $date=$_GET["dateTime"];
        $bdd=base_donnees();
    $insertion=$bdd->prepare('INSERT INTO rdv(DateRdv,IdCompte)
    VALUES(:dateRdv,:id)');
    $insertion->bindValue('dateRdv',$date);$insertion->bindValue('id',$idSuivi);
    $insertion->execute();
...
<body class="animsition">
    <div class="page-wrapper">
        ...
        <div class="page-container">
         ...
         <a class="text-secondary" href="./calendrierMedecin.php?annee=<?=$anneeAvant?>&mois=<?=$moisAvant?>&jour=<?=$jourAvant?>">
           <i class="fas fa-caret-square-left align-self-end" id="avant"></i>
         </a>
         <div>
            <?php echo $jourLundi.$moisLundi."-".$jourDimanche.$moisDimanche;?>
         </div>
         <a class="text-secondary" href="./calendrierMedecin.php?annee=<?=$anneeApres?>&mois=<?=$moisApres?>&jour=<?=$jourApres?>">
                       ...
                       <?php foreach($days as $key=>$jour){
                           if(in_array($jour.$heure,$horaireTravail))
                           {
                                 ...
                                 if($rdv['DateRdv']==dateApresJours($lundi,$key)->format('Y-m-d')." ".$heure.":00")
                                 {
                                    $rserve=1;
                                        ...
                                        if($rdv['statut']=="terminé")
                                        {
                                            ...
                                            <a href="affichageTermine.php?statut=terminé&id='.$rdv['IdCompte'].'&idRDV='.$rdv['idRdv'].'&type=reserve&dateTime='.dateCible($key,$lundi,$heure).'" class="text-dark text-decoration-none text-center">terminé</a></td>';
                                        }
                                        else
                                        {
                                            if($rdv['rdvConfirme'] == 'false'){
                                            
                                                echo'<td  class="bg-danger text-center">réservé</td>';
                                            ...
                                            if($rdv['rdvConfirme']=='true'){
                                                ...
                                                <a href="affiche_reserve.php?id='.$rdv['IdCompte'].'&idRDV='.$rdv['idRdv'].'&type=reserve&dateTime='.dateCible($key,$lundi,$heure).'" class="text-light text-decoration-none text-center">confirmé</a></td>';
                              ...
                              if($rserve==0)
                              {
                                  ...
                                  {
                                    echo'<td class="bg-success text-center">libre</td>';
