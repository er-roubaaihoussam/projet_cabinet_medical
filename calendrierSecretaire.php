...
<body class="animsition">
    <div class="page-wrapper">
        ...
        <div class="page-container">
         ...
         <a class="text-secondary" href="./calendrierSecretaire.php?annee=<?=$anneeAvant?>&mois=<?=$moisAvant?>&jour=<?=$jourAvant?>">
           <i class="fas fa-caret-square-left align-self-end" id="avant"></i>
         </a>
         <div>
            <?php echo $jourLundi.$moisLundi."-".$jourDimanche.$moisDimanche;?>
         </div>
         <a class="text-secondary" href="./calendrierSecretaire.php?annee=<?=$anneeApres?>&mois=<?=$moisApres?>&jour=<?=$jourApres?>">
                           ...
                           if(in_array($jour.$heure,$horaireTravail))
                           {
                              ...
                              foreach($tabRv as $rdv)
                              {
                                 ...
                                 if($rdv['DateRdv']==dateApresJours($lundi,$key)->format('Y-m-d')." ".$heure.":00")
                                 {
                                    ...
                                    if($rdv['statut']=="terminé")
                                    {
                                        ...
                                        <a href="confirmationSecretaire.php?statut=terminé&id='.$rdv['IdCompte'].'&idRDV='.$rdv['idRdv'].'&type=reserve&dateTime='.dateCible($key,$lundi,$heure).'" class="text-dark text-decoration-none text-center">terminé</a></td>';
                                    }
                                    else{
                                        if($rdv['rdvConfirme'] == 'false'){
                                            
                                            echo'<td  class="bg-danger text-center">
                                            <a href="confirmationSecretaire.php?id='.$rdv['IdCompte'].'&idRDV='.$rdv['idRdv'].'&type=reserve&dateTime='.dateCible($key,$lundi,$heure).'" class="text-dark text-decoration-none text-center">réservé</a></td>';
                                        ...
                                        if($rdv['rdvConfirme']=='true'){
                                            ...
                                            <a href="annulerconfirmationSecretaire.php?id='.$rdv['IdCompte'].'&idRDV='.$rdv['idRdv'].'&type=reserve&dateTime='.dateCible($key,$lundi,$heure).'" class="text-light text-decoration-none text-center">confirmé</a></td>';
                              ...
                              if($rserve==0)
                              {
                                  if(dateCibleD($key,$lundi,$heure)>new DateTime("now"))
                                  {
                                    ...
                                    <a href="confirmationSecretaire.php?type=libre&dateTime='.dateCible($key,$lundi,$heure).'" class="text-decoration-none text-light">libre</a></td>';
                                  }
                                  else
                                  echo'<td ></td>';
